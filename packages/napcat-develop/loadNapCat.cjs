...
async function copyAll () {
  ...
  // wrapper.node 依赖 QQ 目录下的其他 DLL，需加入 PATH
  const qqWin64Dir = path.join(BASE_DIR, 'win64');
  process.env.PATH = [BASE_DIR, qqWin64Dir, TARGET_DIR, TARGET_WIN64_DIR, process.env.PATH || ''].join(path.delimiter);
